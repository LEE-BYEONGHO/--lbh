...
ChatGPT가 작성해준 코드를 분석하는 과정에서, 아직 학습하지 못한 함수와 코드의 길이가 길어짐에 따라 해석에 어려움을 겪었다.
또한, 이번 과제를 통해 AI 시대 프로그래밍은 사용자가 단순히 하나부터 열까지 코드를 수동으로 짜는 것이 아닌, AI툴을 활용하여 신속하게 코드를 작성하고, 사용자의 코딩 지식을 바탕으로 AI가 작성한 코드를 검토 및 수정과 적절한 프롬포트를 사용하여 원하는 결과값을 얻게 하는 능력이 중요할 것이라 느꼈다.
...
```
@LEE-BYEONGHO ➜ /workspaces/--lbh (main) $ ./a.out
